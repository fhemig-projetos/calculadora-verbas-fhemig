...
def buscar_cargo(classe, nivel, grau):
    ...
    for c in TABELA_CARGOS:
        if (
            c["classe"].upper() == classe.upper()
            and c["nivel"] == str(nivel)
            and c["grau"].upper() == grau.upper()
        ):
            return c
    return None
...
if "ultimo_resultado" not in st.session_state:
    st.session_state.ultimo_resultado = (
        None  # {"verba", "codigo", "tipo", "valor", "memoria"}
    )
...
def gerar_pdf(historico, ds) -> bytes:
    ...
    valor_style = ParagraphStyle(
        "valor", fontSize=9, textColor=azul, fontName="Helvetica-Bold", leading=12
    )
    ...
    # Cabeçalho
    story.append(
        Paragraph("FHEMIG — Calculadora de Verbas Remuneratórias", titulo_style)
    )
    story.append(
        Paragraph("DIGEPE / CCPT · Conferência de Resumos Funcionais", sub_style)
    )
    ...
    dados_tabela = [
        ...
        [
            ...
            f'{ds["ch_mensal"]:.0f}h' if ds["ch_mensal"] else "—",
            "Vencimento Básico",
            brl(ds["vencimento"]) if ds["vencimento"] else "—",
        ],
    ]
    ...
    for h in historico:
        ...
        linhas.append(
            [
                ...
                h["verba"],
                Paragraph(
                    f'<font color="{cor_tipo.hexval()}">{tipo}</font>', cell_style
                ),
                h["referencia"],
                brl_num(h["valor"]),
            ]
    ...
    linhas.append(
        [
            ...
            "",
            "",
            brl_num(total_vantagens - total_descontos),
        ]
    )
    ...
    rodape = ParagraphStyle(
        "rodape", fontSize=7, textColor=colors.HexColor("#94a3b8"), alignment=TA_CENTER
    )
    story.append(
        Paragraph(
...
with st.expander("📋 Dados do Servidor (cabeçalho do PDF)", expanded=True):
    ...
    c4, c5 = st.columns(2)
    ds["dt_admissao"] = c4.text_input(
        "Data de Admissão", value=ds["dt_admissao"], placeholder="DD/MM/AAAA"
    )
    ds["dt_fim_efetiva"] = c5.text_input(
        "Data Fim Efetiva", value=ds["dt_fim_efetiva"], placeholder="DD/MM/AAAA"
    )

    st.markdown("<hr style='margin:0.6rem 0'>", unsafe_allow_html=True)
    st.caption(
        "**Cargo** — preencha para busca automática do vencimento e carga horária"
    )

    c6, c7, c8 = st.columns(3)
    cargo_classe = (
        c6.text_input(
            "Cargo / Classe", value=ds["cargo_classe"], placeholder="Ex: PENF"
        )
        .upper()
        .strip()
    )
    cargo_nivel = c7.text_input(
    ...
    if cargo_encontrado:
        ...
        ds["ch_semanal"] = c9.text_input(
            "Carga Horária Semanal",
            value=ds.get("ch_semanal", ""),
            placeholder="Ex: 40 Horas Semanais",
        )
        ds["ch_mensal"] = c10.number_input(
            "Carga Horária Mensal",
            value=ds.get("ch_mensal", 0.0),
            min_value=0.0,
            step=1.0,
            format="%.0f",
            key="servidor_ch_mensal",
        )
        ds["vencimento"] = c11.number_input(
            "Vencimento Básico (R$)",
            value=ds.get("vencimento", 0.0),
            min_value=0.0,
            step=0.01,
            format="%.2f",
            key="servidor_vencimento",
        )
...
if verba == "— Selecione uma verba —":
    ...
    tipo_tag = (
        f'<span class="tag-vantagem">VANTAGEM</span>'
        if meta["tipo"] == "Vantagem"
        else f'<span class="tag-desconto">DESCONTO</span>'
    )
    ...
    if verba == "Gratificação de Final de Semana":
        st.markdown(
            '<div class="descricao-verba">Fórmula: (Venc. Básico + Ad. Desempenho) ÷ Carga Horária × Horas Realizadas × 1,0833</div>',
            unsafe_allow_html=True,
        )
        c1, c2 = st.columns(2)
        vb = c1.number_input(
        ...
        hr = c4.number_input(
            "Horas de Final de Semana Realizadas",
            min_value=0.0,
            step=1.0,
            format="%.0f",
            key="gfs_hr",
        ...
        st.markdown(
            '<div class="descricao-verba">Fórmula: Venc. Básico ÷ Carga Horária × Horas Noturnas × 0,20</div>',
            unsafe_allow_html=True,
        )
        c1, c2, c3 = st.columns(3)
        vb = c1.number_input(
        ...
        st.markdown(
            '<div class="descricao-verba">Fórmula: (Venc. Básico + Ad. Desempenho) ÷ Carga Horária × Horas × 1,50</div>',
            unsafe_allow_html=True,
        )
        c1, c2 = st.columns(2)
        vb = c1.number_input(
        ...
        st.markdown(
            '<div class="descricao-verba">Fórmula: (Venc + Ad. Desempenho + Ab. Emergência + Grat. Fim Semana + Ad. Noturno + GRS) ÷ 12 × Nº Meses</div>',
            unsafe_allow_html=True,
        )
        c1, c2, c3 = st.columns(3)
        vb = c1.number_input(
        ...
        grs = c6.number_input(
            "GRS (R$)", min_value=0.0, step=0.01, format="%.2f", key="13_grs"
        )
        mm = st.number_input(
            "Nº de Meses de Direito (1–12)",
            min_value=1,
            max_value=12,
            value=12,
            step=1,
        ...
        st.markdown(
            '<div class="descricao-verba">Fórmula: Valor base ÷ 12 × Nº de Parcelas</div>',
            unsafe_allow_html=True,
        )
        c1, c2 = st.columns(2)
        ...
        if st.button("Calcular", type="primary", use_container_width=True):
            ...
            memoria = [
                f"{brl(base)} ÷ 12 = {brl(base/12)}/parcela",
                f"× {parc} parcelas",
                f"= {brl(resultado)}",
            ]
        ...
        st.markdown(
            '<div class="descricao-verba">Tabela progressiva INSS sobre (13º Salário + GIEFS do 13º)</div>',
            unsafe_allow_html=True,
        )
        c1, c2, c3 = st.columns(3)
        dec = c1.number_input(
        ...
        st.markdown(
            '<div class="descricao-verba">Fórmula: Valor base ÷ 30 × Nº de Dias</div>',
            unsafe_allow_html=True,
        )
        c1, c2 = st.columns(2)
        base = c1.number_input(
        ...
        st.markdown(
            '<div class="descricao-verba">Fórmula: Valor base ÷ 6 × Nº de Parcelas</div>',
            unsafe_allow_html=True,
        )
        c1, c2 = st.columns(2)
        base = c1.number_input(
        ...
        parc = c2.number_input(
            ...
            value=1,
            step=1,
            key="giefs_meses_parc",
        )
        if st.button("Calcular", type="primary", use_container_width=True):
            resultado = (base / 6) * parc
            memoria = [
                f"{brl(base)} ÷ 6 = {brl(base/6)}/parcela",
                f"× {parc} parcelas",
                f"= {brl(resultado)}",
            ]
        ...
        st.markdown(
            '<div class="descricao-verba">Fórmula: Valor base ÷ 3</div>',
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            '<div class="descricao-verba">Fórmula: GRS ÷ 30 × Nº de Dias</div>',
            unsafe_allow_html=True,
        )
        c1, c2 = st.columns(2)
        grs = c1.number_input(
        ...
        st.markdown(
            '<div class="descricao-verba">Fórmula: GRS × Nº de Meses</div>',
            unsafe_allow_html=True,
        )
        c1, c2 = st.columns(2)
        grs = c1.number_input(
        ...
        st.markdown(
            '<div class="descricao-verba">Fórmula: GRS ÷ Carga Horária × Horas de Falta</div>',
            unsafe_allow_html=True,
        )
        c1, c2, c3 = st.columns(3)
        grs = c1.number_input(
        ...
        hf = c3.number_input(
            "Horas de Falta", min_value=0.0, step=1.0, format="%.0f", key="grs_desc_hf"
        )
        if st.button("Calcular", type="primary", use_container_width=True):
            resultado = (grs / ch) * hf
            memoria = [
                f"GRS/hora: {brl(grs/ch)}",
                f"× {hf:.0f} horas",
                f"= {brl(resultado)}",
            ]
        ...
        st.markdown(
            '<div class="descricao-verba">Fórmula: (Salário + Ab. Emergência + Ad. Desempenho + Ad. Noturno + GRS) ÷ 3</div>',
            unsafe_allow_html=True,
        )
        c1, c2, c3 = st.columns(3)
        sal = c1.number_input(
        ...
        grs = c5.number_input(
            "GRS (R$)", min_value=0.0, step=0.01, format="%.2f", key="terco_grs"
        )
        if st.button("Calcular", type="primary", use_container_width=True):
        ...
        st.markdown(
            '<div class="descricao-verba">Fórmula: (Salário + GIEFS + Ab. Emergência + GRS + Ad. Noturno) ÷ 30 × Nº de Dias</div>',
            unsafe_allow_html=True,
        )
        c1, c2, c3 = st.columns(3)
        sal = c1.number_input(
        ...
        gie = c2.number_input(
            "GIEFS (R$)", min_value=0.0, step=0.01, format="%.2f", key="ferias_ind_gie"
        )
        ae = c3.number_input(
            "Abono de Emergência (R$)",
            min_value=0.0,
        ...
        c4, c5, c6 = st.columns(3)
        grs = c4.number_input(
            "GRS (R$)", min_value=0.0, step=0.01, format="%.2f", key="ferias_ind_grs"
        )
        an = c5.number_input(
            "Adicional Noturno (R$)",
            min_value=0.0,
        ...
        st.markdown(
            '<div class="descricao-verba">Fórmula: (Venc + Ad. Desempenho + Ab. Emergência + GRS) ÷ Carga Horária × Horas de Falta</div>',
            unsafe_allow_html=True,
        )
        c1, c2, c3 = st.columns(3)
        vb = c1.number_input(
        ...
        c4, c5, c6 = st.columns(3)
        grs = c4.number_input(
            "GRS (R$)", min_value=0.0, step=0.01, format="%.2f", key="faltas_h_grs"
        )
        ch = c5.number_input(
            "Carga Horária Mensal (h)",
            value=_ch_default,
        ...
        hf = c6.number_input(
            "Horas de Falta", min_value=0.0, step=1.0, format="%.0f", key="faltas_h_hf"
        )
        if st.button("Calcular", type="primary", use_container_width=True):
            base = vb + ad + ae + grs
        ...
        st.markdown(
            '<div class="descricao-verba">Fórmula: (Venc + Ad. Desempenho + Ab. Emergência + GRS) ÷ 30 × Dias de Falta</div>',
            unsafe_allow_html=True,
        )
        c1, c2, c3 = st.columns(3)
        vb = c1.number_input(
        ...
        c4, c5 = st.columns(2)
        grs = c4.number_input(
            "GRS (R$)", min_value=0.0, step=0.01, format="%.2f", key="faltas_d_grs"
        )
        dias = c5.number_input(
            "Dias de Falta",
            min_value=1,
            max_value=30,
            value=1,
        ...
        st.markdown(
            '<div class="descricao-verba">Fórmula: Valor Diário × Quantidade de Dias ou Plantões</div>',
            unsafe_allow_html=True,
        )
        c1, c2 = st.columns(2)
        vd = c1.number_input(
            "Valor Diário (R$)", min_value=0.0, step=0.01, format="%.2f", key="ajuda_vd"
        )
        dias = c2.number_input(
            "Quantidade de Dias ou Plantões",
            min_value=1,
            value=1,
            step=1,
            key="ajuda_dias",
        ...
        st.markdown(
            '<div class="descricao-verba">Fórmula: Valor que gerou o desconto × 4%</div>',
            unsafe_allow_html=True,
        )
        base = st.number_input(
            "Valor que gerou o desconto (R$)",
            min_value=0.0,
            step=0.01,
            format="%.2f",
            key="custeio_base",
        ...
        st.markdown(
            '<div class="descricao-verba">Fórmula: Valor atual × 4,62%</div>',
            unsafe_allow_html=True,
        )
        base = st.number_input(
            "Valor que sofrerá o aumento (R$)",
            value=_vb_default,
            min_value=0.0,
            step=0.01,
            format="%.2f",
        ...
        st.markdown(
            '<div class="descricao-verba">Fórmula: Base de incidência × 3,2%</div>',
            unsafe_allow_html=True,
        )
        base = st.number_input(
            "Valor a sofrer incidência de IPSEMG (R$)",
            min_value=0.0,
            step=0.01,
            format="%.2f",
            key="ipsemg_base",
        ...
        st.markdown(
            '<div class="descricao-verba">Cálculo progressivo conforme tabela INSS do ano de referência</div>',
            unsafe_allow_html=True,
        )
        c1, c2 = st.columns(2)
        base = c1.number_input(
            ...
            step=0.01,
            format="%.2f",
            key="inss_mensal_base",
        )
        ano = c2.selectbox(
            "Ano de Referência", [2026, 2025, 2024], key="inss_mensal_ano"
        )
        if st.button("Calcular", type="primary", use_container_width=True):
            resultado, aliq = calc_inss(base, ano)
            tab = TABELA_INSS[ano]
            memoria = (
                [f"Base: {brl(base)} | Ano: {ano}", f"Tabela INSS {ano}:"]
    ...
    if ur is not None and ur["verba"] == verba:
        ...
        if c2.button("Conferir", use_container_width=True) and val_unidade > 0:
            ...
            if dif < 0.05:
                st.success(
                    f"✅ **Valores conferem.** Calculado: **{brl(ur['valor'])}** | Informado: **{brl(val_unidade)}**"
                )
            else:
                sinal = "a mais" if val_unidade > ur["valor"] else "a menos"
                st.error(
                    f"❌ **Divergência de {brl(dif)} ({sinal}).** Calculado: **{brl(ur['valor'])}** | Informado: **{brl(val_unidade)}**"
                )
...
if not st.session_state.historico:
    st.info(
        "Nenhum cálculo adicionado ainda. Após calcular uma verba, clique em **➕ Adicionar à lista**."
    )
else:
    # Tabela resumo
    import pandas as pd
    ...
    total_v = sum(
        h["valor"] for h in st.session_state.historico if h["tipo"] == "Vantagem"
    )
    total_d = sum(
        h["valor"] for h in st.session_state.historico if h["tipo"] == "Desconto"
    )
    c1, c2, c3 = st.columns(3)
    ...
    if col_pdf.button("📥 Gerar PDF", type="primary", use_container_width=True):
        pdf_bytes = gerar_pdf(
            st.session_state.historico, st.session_state.dados_servidor
        )
