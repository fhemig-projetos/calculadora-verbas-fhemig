...
class SelecaoVerba:
    ...
    def render(self):
        ...
        verba_input = st.selectbox(
            "Verba", options=opcoes, label_visibility="collapsed"
        )
        ...
        st.markdown(
            f'**Verba {verba_meta["codigo"]}** &nbsp; {tipo_tag}',
    ...
    def _render_calculadora(
        self, calculadora: CalculadoraVerba, verba_meta: dict, nome_verba: str
    ):
        # Exibe a fórmula
        ...
        if (
            st.session_state["ultima_referencia_cabecalho"] is not None
            and st.session_state["ultima_referencia_cabecalho"] != referencia_cabecalho
        ):
            for campo_cabecalho in (
                "vencimento_basico",
            ...
            nonce = st.session_state[
                "verba_nonce"
            ]  # atualiza a variável já usada nas keys abaixo
        ...
        for i, campo in enumerate(calculadora.campos_necessarios):
            ...
            if campo == "vencimento_basico":
                ...
                indice_default = (
                    opcoes_ch.index(valor_default) if valor_default in opcoes_ch else 0
                )
            ...
            ## Campos de selectbox → índice vem do persistido (se a opção existir)
                ...
                if valor_persistido in opcoes_ano:
                    ...
                    ano_default = (
                        date.today().year
                        if date.today().year in opcoes_ano
                        else opcoes_ano[-1]
                    )
                ...
                if nome_verba in [
                    ...
                    opcoes_grs = [
                        "Não faz jus (R$ 0,00)",
                        "Risco Médio (R$ 160,20)",
                        "Risco Alto (R$ 320,40)",
                    ]
                valor_persistido = persistidos.get(campo)
                indice_default_grs = (
                    opcoes_grs.index(valor_persistido)
                    if valor_persistido in opcoes_grs
                    else 0
                )
            ...
            ## tipo Vantagem, exceto Ajuda de Custo e as verbas de 13º, que entram no INSS do 13º)
                ...
                valor_default = sum(
                    (
                        ...
                        and item.get("nome_verba") not in NOMES_EXCLUIDOS_INSS
                    ),
            ...
            ## Campos do Histórico → histórico primeiro, persistido como fallback
                ...
                nome_alvo_dict = (
                    {  ## tentar melhorar essa lógica depois para ficar mais eficiente
    ...
    def _render_resultado(self):
        ...
        st.metric(
            label="Resultado",
            value=FormatadorCampos.brl(ur["valor"]),
        ...
        if st.button(
            "➕ Adicionar à lista", type="secondary", use_container_width=True
        ):
    ...
    def _render_competencia(self, nome_verba: str):
        ...
        if nome_verba in (
            ...
            ano = col_ano.selectbox(
                "Ano", options=range(2000, 2031), index=ano_default - 2000
            )
    ...
    def _render_historico(self):
        ...
        vantagens = sum(
            item["valor"] for item in historico if item.get("tipo") == "Vantagem"
        )
        descontos = sum(
            item["valor"] for item in historico if item.get("tipo") == "Desconto"
        )
        ...
        with col_btn1:
            if st.button(
                "🗑️ Remover último", type="secondary", use_container_width=True
            ):
